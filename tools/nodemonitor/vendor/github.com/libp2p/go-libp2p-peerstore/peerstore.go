...
type KeyBook interface {
	PubKey(peer.ID) *ecdsa.PublicKey
	AddPubKey(peer.ID, *ecdsa.PublicKey) error

	PrivKey(peer.ID) *ecdsa.PrivateKey
	AddPrivKey(peer.ID, *ecdsa.PrivateKey) error
}

type keybook struct {
	pks map[peer.ID]*ecdsa.PublicKey
	sks map[peer.ID]*ecdsa.PrivateKey
...
func (kb *keybook) PubKey(p peer.ID) *ecdsa.PublicKey {
	kb.RLock()
	pk := kb.pks[p]
	kb.RUnlock()
	if pk != nil {
...
func (kb *keybook) AddPubKey(p peer.ID, pk *ecdsa.PublicKey) error {
...
func (kb *keybook) PrivKey(p peer.ID) *ecdsa.PrivateKey {
	kb.RLock()
	sk := kb.sks[p]
	kb.RUnlock()
	return sk
...
func (kb *keybook) AddPrivKey(p peer.ID, sk *ecdsa.PrivateKey) error {
